...
abstract public class LinearClassifier {
	 ...
	 * See AIMA p. 724.
	 */
	public void train(List<ExampleLC> examples, int nsteps, LearningRateSchedule schedule) {
		
		Random random = new Random();
		int n = examples.size();
		for (int i=1; i <= nsteps; i++) {
			int j = random.nextInt(n);
			ExampleLC ex = examples.get(j);
	 ...
	 */
	public void train(List<ExampleLC> examples, int nsteps, double constant_alpha) {
	 ...
	 */
	protected void trainingReport(List<ExampleLC> examples, int stepnum, int nsteps) {
	 ...
	 */
	public double squaredErrorPerSample(List<ExampleLC> examples) {
		double sum = 0.0;
		for (ExampleLC ex : examples) {
	 ...
	 */
	public double accuracy(List<ExampleLC> examples) {
		int ncorrect = 0;
		for (ExampleLC ex : examples) {
			double result = eval(ex.inputs);
			if (result == ex.output) {
